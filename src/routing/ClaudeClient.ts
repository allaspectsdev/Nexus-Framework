...
/** Core streaming implementation — separated for retry wrapping. */
async function* streamImpl(client: Anthropic, options: ModelCallOptions): AsyncGenerator<StreamEvent> {
...
export function createClaudeClient(apiKey: string): ModelClient {
  ...
  return {
    ...
    async *stream(options: ModelCallOptions): AsyncGenerator<StreamEvent> {
      yield* withRetry(() => streamImpl(client, options), { signal: options.signal })
    },
